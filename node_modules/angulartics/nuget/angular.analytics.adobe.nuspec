<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd">
    <metadata>
        <id>Angular.Analytics.Adobe</id>
        <version>0.15.19</version>
        <title>Angulartics for Adobe Analytics</title>
        <authors>luisfarzati,ADAPTByDesign</authors>
        <projectUrl>http://luisfarzati.github.io/angulartics/</projectUrl>
        <iconUrl>http://luisfarzati.github.io/angulartics/images/angularjs.png</iconUrl>
        <requireLicenseAcceptance>false</requireLicenseAcceptance>
        <description>Adobe Analytics module for Angulartics.
See Angular.Analytics.Core for more details

[NuGet packaging by ADAPTByDesign]
</description>
        <summary>Adobe Analytics for AngularJS applications</summary>
        <releaseNotes />
        <copyright />
        <tags>angularjs analytics adobe</tags>
        <dependencies>
            <dependency id="Angular.Analytics.Core" version="0.15.19" />
        </dependencies>
    </metadata>
    <files>
        <file src="..\src\angulartics-adobe.js" target="content\Scripts\angulartics-adobe.js" />
        <file src="..\dist\angulartics-adobe.min.js" target="content\Scripts\angulartics-adobe.min.js" />
    </files>
</package>